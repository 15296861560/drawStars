...
export function listConversations(curPage = 1, pageSize = 50) {
  return get<PageResult<ImConversation>>(
    { curPage, pageSize },
    '/im/conversations'
  )
}
...
export function markRead(conversationId: string, seq: number) {
  return post<{ unreadCount: number }>(
    { seq },
    formatPath('/im/conversations/:id/read', { id: conversationId })
  )
}

export function pinConversation(conversationId: string, pinned: boolean) {
  return post<ImConversation>(
    { pinned },
    formatPath('/im/conversations/:id/pin', { id: conversationId })
  )
}
...
export function muteConversation(conversationId: string, muted: boolean) {
  return post<ImConversation>(
    { muted },
    formatPath('/im/conversations/:id/mute', { id: conversationId })
  )
}
...
export function saveDraft(conversationId: string, draft: string) {
  return post<ImConversation>(
    { draft },
    formatPath('/im/conversations/:id/draft', { id: conversationId })
  )
}
...
export function deleteConversation(conversationId: string) {
  return post<{}>(
    {},
    formatPath('/im/conversations/:id/delete', { id: conversationId })
  )
}
