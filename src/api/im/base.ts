...
/** POST：params 走 body */
export function post<T = unknown>(params: Record<string, unknown>, url: string): Promise<ApiResult<T>> {
  return $axios(params, url) as unknown as Promise<ApiResult<T>>
}

/** GET：params 走 query */
export function get<T = unknown>(params: Record<string, unknown> = {}, url: string, options: Record<string, unknown> = {}): Promise<ApiResult<T>> {
  return $axiosGet(params, url, options) as unknown as Promise<ApiResult<T>>
}

/** DELETE：params 走 body（后端 @Delete 路由从 @Body 读取） */
export function del<T = unknown>(url: string, body: Record<string, unknown> = {}): Promise<ApiResult<T>> {
  return requestMethod<T>('put-ignored', url, body, 'delete')
}

/** PUT：params 走 body（后端 @Put 路由） */
export function put<T = unknown>(url: string, body: Record<string, unknown> = {}): Promise<ApiResult<T>> {
  return requestMethod<T>('put', url, body, 'put')
}

function requestMethod<T>(_tag: string, url: string, body: Record<string, unknown>, method: 'delete' | 'put'): Promise<ApiResult<T>> {
  return new Promise(resolve => {
    const apiInfo = apiInfoStore()
    const fullUrl = `${apiInfo.getURL.value}${url}`
    const cfg = { data: body, timeout: 30000 }
    const call = method === 'delete' ? requests.delete(fullUrl, cfg) : requests.put(fullUrl, body, { timeout: 30000 })
    call
      .then(res => {
...
export function formatPath(tpl: string, params: Record<string, string | number>): string {
  let out = tpl
