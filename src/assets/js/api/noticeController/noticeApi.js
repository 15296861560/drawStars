...
export default {
  ...
  queryNoticeList: param => {
    return commomApi.queryInfoList(
      param,
      tableName,
      Object.keys(param).filter(
        item => item !== 'pageSize' && item !== 'curPage'
      )
    )
  },
  getNoticeCount: param => {
    const paramKeys = Object.keys(param).filter(
      item => item !== 'pageSize' && item !== 'curPage'
    )
    return commomApi.getInfoCount(
      paramKeys,
