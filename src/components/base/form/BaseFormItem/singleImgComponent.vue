...
const imgUploadRef = ref<ElementPlus.UploadInstance>()
...
const handleExceed: ElementPlus.UploadProps['onExceed'] = files => {
  imgUploadRef.value!.clearFiles()
  const file = files[0] as ElementPlus.UploadRawFile
  file.uid = ElementPlus.genFileId()
  imgUploadRef.value!.handleStart(file)
}

const handleRemove = (_file: ElementPlus.UploadFile) => {
  imgUploadRef.value!.clearFiles()
  field.value = ''
}

const handlePictureCardPreview = (file: ElementPlus.UploadFile) => {
  dialogImgUrl.value = file.url!
  dialogImgVisible.value = true
}

const handleDownload = async (_file: ElementPlus.UploadFile) => {
